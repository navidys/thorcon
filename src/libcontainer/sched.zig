...
pub fn unshare() void {
    std.log.debug("create isolated namespace", .{});

    const unshare_flags = linux.CLONE.NEWNS | linux.CLONE.NEWNET | linux.CLONE.NEWUTS;
...
pub fn clone(f: anytype, args: anytype) !usize {
    ...
    switch (linux.E.init(pid)) {
        .SUCCESS => std.log.debug("child clone pid {d}", .{pid}),
        else => |err| {
