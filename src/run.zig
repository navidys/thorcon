...
pub fn exec(_: std.mem.Allocator, iter: *std.process.ArgIterator, main_args: anytype) !void {
    ...
    while (parser.next() catch |err| {
        ...
        switch (arg.param.id) {
            ...
            else => return errors.Error.InvalidContainerRunOptions,
        }
    }

    const runOpts = libcontainer.run.RunOptions{
        .name = containerName,
        .noPivot = noPivot,
        .bundleDir = bundleDir,
        .spec = spec,
    ...
    try libcontainer.run.runContainer(main_args.args.root, runOpts);
}

fn usage() !void {
    const stdout = std.io.getStdOut().writer();
    try stdout.print("Usage: run [OPTION...] CONTAINER\n\n", .{});
